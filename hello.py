...
def do_search2() -> dict:
    ...
    result_list = list(
        zip_longest(sorted_SE, sorted_SI, sorted_TE, sorted_TI, sorted_FE, sorted_FI, sorted_NE, sorted_NI,
                    fillvalue=['0', 0]))
    result_list = list(map(list, result_list))  # лист листов
    ...
    log_request(text, new_result_list)
    return render_template('results3.html', title='', max=max(bar_values), labels=bar_labels, values=bar_values,
                           # # the_results8=sorted_NI,
                           # the_results8=new_result_list,
                           table=new_result_list,
                           )
